...
int main(int argc, char *argv[]){
	if(argc != 2) {
    	cerr << "USAGE: ./verify_mp10 input.txt" << endl; 
    	return -1;
  	}
	cout << " \n\n ------------------- Begin Verifying MP10 ---------------------" << endl;
	ifstream ifs (argv[1], std::ifstream::in);
    if(ifs.fail()){
        std::cout << "ERROR: " << argv[1] << " does not exist " << endl;
        cout << "-------------------- Stop Verifying MP10 ----------------------" << endl << endl << endl;
        return 0;
    }
    if(!CheckFile(argv[1])){
	...
	cout << "Your total Score for MP10: " << mv_p + ma_p + name_p + rec_p + cir_p + s_p + rp_p + cs_p << "/100" << endl;
	cout << "-------------------- End Verifying MP10 ----------------------" << endl << endl << endl;
